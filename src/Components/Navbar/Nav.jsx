...
const Nav = ({ theme, setTheme }) => {
    ...
    return (
        <>
            <nav className="fixed top-0 left-0 w-full z-50 bg-base-100/20 backdrop-blur-md border-b border-base-300/30">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="h-20 flex items-center justify-between">
                        ...
                        >
                            <GoDotFill className="text-2xl text-emerald-500 transition-transform duration-300 group-hover:scale-125" />
                                    ...
                                    >
                                        {({ isActive }) => (
                                            <>
                                                ...
                                                <span
                                                    className={`absolute -bottom-1 left-0 h-[2px] bg-emerald-400 rounded-full transition-all duration-300 ${
                                                        isActive
                                                            ? 'w-full'
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                <span
                                    className={`absolute top-0.5 h-6 w-6 rounded-full bg-base-100 shadow-md flex items-center justify-center transition-all duration-300 ${
                                        isDark
                                            ? 'translate-x-8'
                                            : 'translate-x-0.5'
                                    }`}
            ...
            {/* Mobile Drawer */}
            <div
                className={`fixed top-0 right-0 h-screen w-72 bg-base-100 backdrop-blur-xl border-l border-base-300 z-50 transition-transform duration-300 ${
                    isOpen
                        ? 'translate-x-0'
                        : 'translate-x-full'
                }`}
